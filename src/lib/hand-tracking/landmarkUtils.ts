...
import type { Landmark } from "../types/hand.js";
import { FINGERTIP_INDICES, LandmarkIndex } from "../types/hand.js";
...
export const getHandOpenness = (landmarks: Landmark[]): number => {
  ...
  const totalDistance = fingertips.reduce(
    (sum, tip) => sum + distance2D(tip, palmCenter),
    0
  );
...
export const getFingerCurl = (
  ...
  const fingerIndices: Record<
    string,
    [LandmarkIndex, LandmarkIndex, LandmarkIndex, LandmarkIndex]
  > = {
    thumb: [
...
/**
 * Get the angle of the line from thumb tip to index tip
 * Returns normalized value 0-1 where:
 * - 0 = pointing right
 * - 0.25 = pointing up
 * - 0.5 = pointing left
 * - 0.75 = pointing down
 * - 1 = pointing right again (full circle)
 */
export const getPinchAngle = (landmarks: Landmark[]): number => {
  const thumb = landmarks[LandmarkIndex.THUMB_TIP];
  ...
  // Normalize to 0-1 range (full circle)
  const normalized = (angle + Math.PI) / (2 * Math.PI);
...
/**
 * Get normalized pinch distance between thumb and index finger
 * Returns value 0-1 where 0 is touching and 1 is max spread
 */
export const getNormalizedPinchDistance = (landmarks: Landmark[]): number => {
  const distance = getPinchDistance(landmarks, LandmarkIndex.INDEX_TIP);
  // Typical pinch distances range from ~0.02 (touching) to ~0.25 (spread)
  // Normalize to 0-1 range
  const normalized = (distance - 0.02) / 0.23;
  return Math.max(0, Math.min(1, normalized));
};
